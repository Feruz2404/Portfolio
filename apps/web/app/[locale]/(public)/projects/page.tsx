...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale }> }): Promise<Metadata> {
  ...
  return buildMetadata({ locale, path: "/projects", title: t("title"), description: t("intro") });
}
    ...
    <>
      ...
      <Section width="wide" className="border-t border-line pt-12">
        {projects.length === 0 ? (
          ...
          <ProjectsGrid projects={projects} allLabel={t("filterAll")} viewLabel={common("viewProject")} />
        )}
      </Section>
    </>
  );
